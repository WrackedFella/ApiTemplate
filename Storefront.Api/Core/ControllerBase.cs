...
namespace Storefront.Api.Core
{
	  ...
	  where TModel : ModelBase
  {
	...
	[HttpGet("{id}")]
	public async Task<ActionResult<TModel>> Get(string id)
	{
	  if (!Guid.TryParse(id, out var guid))
	  {
		return BadRequest();
	  }
	  var record = await this.Context.Set<TEntity>().FindAsync(guid);
	  if (record == null)
	  {
		return NotFound();
	  }
	...
	[HttpPut("{id}")]
	public async Task<ActionResult<AcceptedAtRouteResult>> Put(string id, [FromBody] TModel model)
	{
